...
20. **Phase 12's KL estimator exploded on the β=0.001 relaunch; fixed by dropping β, not patching the estimator (2026-09-10).** GDPO-from-RS-SFT's first launch (`gdpo-qlora-gdpo-phase12-rssft`) was killed at step 96: KL rose 1.32→97.2→**1.99e11** across steps 25/50/75, loss became ~99.99% KL penalty (β·KL≈1.99e8 vs logged loss 2.07e8), grad_norm collapsed 616→1.84→0.072 (one outlier update, then no training signal at all), reward declining monotonically 3.88→3.80→3.68. **Mechanism**: TRL 1.8.0's k3/Schulman KL estimator (`exp(ref_logp−policy_logp) − (ref_logp−policy_logp) − 1`) is unbiased but unbounded above with infinite variance under heavy-tailed log-ratios — one token the policy has driven far below the reference's probability can dominate a batch mean. `epsilon_high=5.0` clips the policy-loss importance ratio, never this KL term. Root-cause token hunt (`research/diagnose_kl_blowup.py`, 476 completions, RS-SFT-from-base as an approximate stand-in for the unrecoverable step-75 policy): max r=15.88 nats, short of the ~35–37 a single token would need at face value — inconclusive on magnitude, but 7/10 top hits are digit tokens embedded in fractional-stoichiometry arithmetic, directionally consistent with the fractional/doped stratum's known fragility. **Fix: β=0 for the relaunch**, not a per-token clamp — Arm A's own KL (0.001–0.003) already contributed a negligible ~2e-6 to its loss, DAPO drops the KL term entirely in long-horizon RLVR once the policy is meant to move from the reference, and finding 17 already established the reference doesn't bind at this β anyway. Relaunch was clean: zero `train/kl` logged at any point, grad_norm never exceeded ~0.45. Source: `docs/phases/PHASE12_RESULTS.md`.

21. **Phase 12 pre-launch diagnostics: a real, predicted-in-advance `operation_order` variance drop, and a parse-failure fix that halved fractional-stratum attrition (2026-09-08/09).** (1) Smoke gate (20 steps): `operation_order` read exactly 1.000 across all 311 scored completions — zero variance against a historical zero-std-group rate of 78.8% (P(zero variance | baseline) ≈ 0.788³⁹ ≈ 1e-4 at ~39 groups). Mechanism identified *before* the full run: RS-SFT trained on validator-filtered survivors at bar 0.9, so every training example already had good operation ordering — **on-policy RL cannot revive a channel it never fails.** Predicted "`operation_order` stays dead throughout" in advance; confirmed exactly at checkpoint 300 (0.000 on all 4 probe readings). (2) Parse-failure investigation: an 18.2% smoke-sample parse-failure rate, concentrated on fractional/doped targets (the same stratum finding 10 flagged), traced to two `parse_completion` bugs (invalid-JSON fraction literals; naive brace-span crossing multiple JSON objects) and repaired, dropping the rate to 12.4% (69→47/380). 46/47 residual failures are `max_new_tokens=8192` truncations, disproportionately on fractional/doped targets — a second, validator-independent confirmation that this stratum needs more `<think>` budget. **Decision: launch at 12.4%, do not raise the token cap** — instrumented live via a per-stratum W&B alert instead, to avoid moving two variables against Arm A at once (init and token budget). Source: `docs/phases/PHASE12_RESULTS.md`.

22. **Phase 12 settled: GDPO from RS-SFT AMPLIFIED the predicted-route rate — the project's first positive RL result (2026-09-16/19).** Checkpoint 300, the pre-registered stopping point (not resumed toward 600, to avoid turning one pre-registered read into two with the chance to pick the favorable one). ASTRAL n=32:

    | | RS-SFT (init) | GDPO-phase12-β0 (ckpt 300) |
    |---|---|---|
    | predicted hits | 10/35 | **14/35** |
    | conventional hits | 17/35 | 17/35 (flat, McNemar p=1) |
    | mean reward | 0.972 | 0.982 |
    | mean max-T | 934.2 °C | 976.1 °C (+41.9, vs full-SFT→GDPO's +53.9) |

    **VERDICT: AMPLIFIED** (14/35 clears the pre-registered >12/35 bar) — RL moved the policy further toward ASTRAL-predicted routes than RS-SFT's own init, without regressing toward convention (unlike full-SFT→GDPO's 17→31 pull back). McNemar on the 4 discordant gains (`BaLiBO3`, `KLi(PO3)2`, `Li2TiSiO5`, `NaSrBO3` — all one-directional, 0 losses) is not significant at the conventional bar (one-sided exact p=0.0625): report as "consistent with amplification, not yet statistically decisive," never as confirmed. **A specific alternative explanation was checked and cleared**: Phase 13's ammonium/balance-solver bug plausibly shaped which routes passed RS-SFT's filter (ammonium-precursor prevalence 12.14%→2.03% in RS-SFT's survivors, a proxy comparison), raising the concern that RS-SFT's gain was an "accidental ablation" of ammonium-affected targets' conventional routes. Direct check (`research/ammonium_ablation_check.py`): 3 of the 4 GDPO gains are in the ammonium-**un**affected group (base/RS-SFT/GDPO there: 47.1%→52.9%→70.6%); the ammonium-affected group is flat and low throughout (11.1%→5.6%→11.1%). **The improvement trajectory sits exactly where the bug has no purchase** — the alternative explanation is ruled out, not merely unaddressed. This is also Phase 13's gate answer: it gave Phase 13's comparator reward a genuine positive result to try to build on, so finding 18 (RS-SFT retention) does not stand alone as the project's only positive result. Source: `docs/phases/PHASE12_RESULTS.md`.

23. **Phase 13 stopping decision: the comparator's discriminative power on ASTRAL's 35 pairs is structurally zero; Phase 14 does not launch (2026-09-19).** Iteration 1's headline number (N_pref=17/35) was uninformative by construction — a comparator that unconditionally prefers "predicted" scores 35/35 on this metric, so 17 is not evidence of working physics. Traced to `C7_gas_evolution`'s MAD-scale calibration failure: raw diff always ∈{0.5,1.0,1.5}, MAD scale 2.085e-5 (computed from a corpus where most completions release no gas), inflating this one channel ~50,000x and producing 99.99% of every nonzero margin — **finding 20's failure mode in a different file**: a robust-scale estimator silently becoming the entire signal. **The deeper problem survives the scale-calibration fix (iteration 2)**: every one of ASTRAL's 35 pairs has traditional=exactly-3-precursors, predicted=exactly-2 (baked into ASTRAL's own selection principle, not an artifact), so any precursor-count/class-sensitive channel is a **deterministic function of the label**, not an independent signal. Corrected iteration-2 primary endpoint: N_pref=35/35 (sign never varies), so sign agreement collapses to the dataset's own base rate (20/35=57.1%) — **the comparator's demonstrated discriminative power on these 35 pairs is zero.** Confirmed as a hard data-availability ceiling, not a shortcut: only 35 such pairs exist in this repository, and ASTRAL's own supplementary information does not contain a matched per-temperature sweep either (checked directly against the full 75-page arXiv SI — the only temperature data is an untabulated heatmap image). **Per the pre-registration's own rule, the gate was not cleared and Phase 14 does not launch** — a pre-registered stopping decision, not an abandoned effort. ARROWS³ (Nat. Commun. 2023, positive-and-negative robot-lab outcomes) was named as the concrete future-work path this data-availability ceiling requires — **picked up directly as Phase 15 Task 4 (finding 27)**. Source: `docs/phases/PHASE13_RESULTS.md`.

24. **Phase 15: the bare-oxide/carbonate shift is real, larger than the ASTRAL metric, and correctly computed but wrong-construct (2026-09-20 to 24).** GDPO's largest distributional effect on ASTRAL generations is invisible to the headline hit-rate metric. Bare-alkali-oxide share climbs base 8.6% → full SFT 0.5% → RS-SFT 25.1% → GDPO 56.3%, with carbonate share moving inversely (74.7% → 98.5% → 48.0% → 19.1%), while ASTRAL-predicted-set share stays flat at 2–4% throughout. The mechanism is fully traced: within 50 GDPO training groups pitting a bare-oxide completion against a carbonate one, `thermodynamic_favorable` is the only channel with a real within-group z-gap (1.29 SD), and this reflects a genuinely-computed, Gibbs-corrected ΔG(T) (Bartel descriptor + NIST-JANAF gas thermochemistry, CO₂ entropy included) that favors bare oxides by 0.19–0.32 eV/atom and **survives a finite-temperature correction** (shrinks 11–25%, never reverses, across every computable one of 5 matched pairs). This is not a validator bug. But real practice does the opposite: Lee et al. 2025 (*Sci. Data* 12:1969, 80,806 records) shows chemists choose carbonate over bare oxide 60.0% vs 2.5% of the time among alkali-containing targets (n=13,849 triples) — a ~24x preference — and among the 20/35 ASTRAL targets appearing in Lee's corpus, **zero** literature routes use a bare alkali oxide at all. Critically, this avoidance is **not** explained by worse published outcomes: no significant impurity-rate difference (pooled diff −0.3%, 95% CI [−3.0%,+2.3%], n=338 vs 7,616; target-matched +0.5%, CI [−3.9%,+5.0%], n=141 vs 1,295) — though outcome-selection in published literature means this doesn't settle unselected-attempt success rates. **GDPO is optimizing a real, correctly-computed thermodynamic quantity that turns out not to be the construct that determines real synthesis success** — kinetics, handling, and nucleation constraints the validator has no channel for. Source: `docs/phases/PHASE15_DISTRIBUTIONAL.md`, `docs/phases/PHASE15_EXTERNAL.md`.

25. **Phase 15: two distinct precursor-shift mechanisms, only one a bug.** GDPO's two largest precursor-class shifts on ASTRAL both move away from real chemist practice, for unrelated reasons. (1) **Ammonium-phosphate avoidance is a software bug**: the balance solver never tries a candidate volatile set containing NH₃ without N₂, so ammonium-salt routes fail `stoichiometry` catastrophically (gap −0.905, pass@0.9 9.5% n=137 vs H₃PO₄'s 99.1% n=215) — confirmed directly on base's own spontaneous ASTRAL generations, the broadest confirmation yet of the Phase 13 diagnosis. Real chemists prefer ammonium phosphate 10:1 over H₃PO₄+P₂O₅ (Lee et al., n=2,564 vs 263); RS-SFT/GDPO trained away from it for a software reason, not a chemistry one (share 23.8%→82.6%→4.9%→2.1%, base/SFT/RS-SFT/GDPO). (2) **Bare-oxide preference is not a bug** (finding 24) — a real, correctly-computed thermodynamic quantity that simply isn't what determines synthesis success. Source: `docs/phases/PHASE15_DISTRIBUTIONAL.md`, `docs/phases/PHASE15_EXTERNAL.md`.

26. **Phase 15: RS-SFT does not amplify its own training-set bias; GDPO does.** Correcting an earlier flawed measurement (which crossed prompt distributions): RS-SFT's ASTRAL-inference-time bare-oxide share (25.8%, n=1,134 triples) sits almost exactly at its own training set's share (28.8%, n=66 triples, only 64/295 training targets alkali-relevant) — no amplification between training composition and RS-SFT's own generation behavior. **The real amplification happens at the GDPO step**: RS-SFT-on-ASTRAL (25.8%) → GDPO-on-ASTRAL (56.9%) is a genuine ~2.2x further increase, driven by the within-group `thermodynamic_favorable` reward advantage (finding 24's z-gap of 1.29 SD), positive in every one of 5 step-buckets across training (too few groups per bucket, 4–17, to confirm a growth trend). A split-half noise-floor control confirms the real distributional shift happens at base→RS-SFT (Jaccard 0.328–0.484, clearly below both models' self-noise floors of 0.52–0.68); RS-SFT→GDPO's Jaccard (0.525) is statistically indistinguishable from GDPO's own resampling noise — **GDPO redistributes probability mass within RS-SFT's existing support, it does not add qualitatively new solutions.** Source: `docs/phases/PHASE15_DISTRIBUTIONAL.md`.

27. **Phase 15: two independently pre-registered external verifier gates, neither beats a trivial baseline.** The validator and `core/comparator.py` were both tested against real, independent data outside the training loop, pre-registered before scoring. **ARROWS³ robot-lab data** (4,870 real synthesis outcome pairs, YBCO/LTOPO/NTMO): on the primary YBCO endpoint (n=1,915, |Δ|≥5wt%), the validator's GDPO-vote rule (0.616 agreement) does not significantly beat the best trivial baseline (carbonate-free, 0.583; diff +0.033, 95% CI [−0.043,+0.112]), and the comparator (0.474) scores **below chance** and below every baseline (diff −0.109, CI [−0.226,+0.007]). Secondary replications on LTOPO/NTMO also fail to beat baseline (underpowered, n=67–77). A direct robot-lab Ba-source controlled test finds no significant yield advantage for the verifier's preferred source (mean diff +5.31 wt%, 95% CI [−3.36,+14.60], n=10 groups; sign test exactly 9/9). Diagnosis: the comparator's gates (not its scored channels) decide most pairs, and decide them badly (gate-decided agreement 0.445, below chance, n=943, vs channel-decided 0.513, n=704) — 16/47 (34%) of YBCO's `precursors_exist` gate failures trace entirely to `Y2(CO3)3`'s absence from Materials Project, a real, comparably-performing compound (finding 29). The best single channel (`thermodynamic_favorable`, 71.1% agreement alone) is 88% redundant with the carbonate-free baseline on pairs where both are decisive, and shows no signal beyond the carbonate dimension once it's held constant (agreement 0.522, CI [0.495,0.557], centered on chance). "Avoid carbonates" itself beats chance significantly (diff +0.083, CI [+0.010,+0.154]) — neither verifier improves on a two-line heuristic. Combined with ranker v2 (finding 19, ASTRAL, failed its gate): every external verifier evaluation this project has run has failed to clear a baseline-matched bar with a CI that excludes zero. Source: `docs/phases/PHASE15_ARROWS_RESULTS.md`, `docs/phases/PHASE15_ARROWS_INVENTORY.md`.

28. **Phase 15: policy-level teacher-forcing confirms the carbonate shift is class-based and yield-blind, with caveats.** Teacher-forcing exact log-probabilities (no sampling) of the precursor segment through base/RS-SFT/GDPO-300/full-SFT on 75 real ARROWS³ routes, pre-registered, found: (1) a leash test was **degenerate as specified** — ε=−20 was a specification error, miscalibrated for a ~51-token summed span, not a null result. (2) A quality-alignment test was **uninformative**, CI [−2.887,+2.585] too wide to distinguish an effect from none. (3) At the first precursor-token position, base already assigns non-trivial probability to each target's eventually-best route (3/3 ARROWS targets). (4) **The headline result**: `Δlog p(GDPO−RS-SFT)` on real measured-yield routes depends significantly on carbonate status (coefficient −1.979, 95% CI [−3.554,−0.352], n=75, robust to target fixed effects, a broader carbonate definition, and a length covariate — the raw coefficient is if anything conservative) but shows **no detectable dependence on the actual measured yield** (coefficient +0.020, CI [−0.006,+0.043]) once carbonate status is in the model — the CI does not exclude a moderate yield effect, though. This is a policy-level (not validator-score) signature consistent with GDPO inheriting `thermodynamic_favorable`'s carbonate preference; it does not itself establish the causal path. **Limitation**: every number above conditions on an empty `<think>` block, so absolute ranks/log p describe the answer distribution with reasoning skipped, not real generation behavior — an exploratory follow-up found GDPO's local, empty-think preference still favors the carbonate continuation at rank 1 on 3 held-out targets, contrasting with its 19.1% carbonate share in real *sampled* generations (finding 24) [E, untested: the shift may be expressed partly through the reasoning trace this probe cannot see]. Source: `docs/phases/PHASE15_DOSE_RESULTS.md`.

29. **Phase 15: `precursors_exist` measures database coverage, not chemical soundness.** Auditing every ARROWS³ gate failure by category found zero notation/parsing failures (category b is empty) and that 100% of YBCO's 16 `precursors_exist` failures trace to a single precursor, `Y2(CO3)3`, confirmed absent from Materials Project's formula set by direct membership check — while all 29 other distinct precursor formulas across all three targets (including parenthetical forms, peroxides, and complex ammonium-phosphate notations) parse cleanly and are present in MP. `Y2(CO3)3` appears in 16/47 (34%) of YBCO's real precursor sets and, at matched temperature, performs comparably to or slightly better than alternative Y-sources (e.g. 900°C: 66.2% n=16 vs 62.1% n=31) — a real, working, published-robotic-synthesis compound. Materials Project is a DFT-computed database, not an exhaustive chemistry registry; a third of this dataset's precursor space is invisibly excluded from `precursors_exist`-gated scoring for a database-completeness reason unrelated to real performance. Source: `docs/phases/PHASE15_ARROWS_RESULTS.md`.
...
8. **Phase 12 (GDPO from RS-SFT) launched, hit a KL-estimator blowup, fixed by dropping β, and settled AMPLIFIED** (findings 20–22) — the project's first positive RL result: 14/35 ASTRAL predicted-set hits vs RS-SFT's own 10/35, checked against and cleared of an "accidental ablation" alternative explanation via the ammonium-precursor bug.
9. **Phase 13 (learned pairwise comparator) ran two iterations and stopped on its own pre-registered gate** (finding 23): ASTRAL's 35 pairs are structurally incapable of evaluating a precursor-count-sensitive comparator (traditional is always 3 precursors, predicted always 2, baked into ASTRAL's own selection principle) — a hard data-availability ceiling, not underpowered noise. Phase 14 does not launch. ARROWS³ named as the concrete path forward.
10. **Phase 15 picked up exactly that path, and is now closed** (findings 24–29): validated the bare-oxide/carbonate and ammonium-phosphate shifts against real literature (Lee et al. 2025) and real robot-lab data (ARROWS³), pre-registered and ran the ARROWS³ verifier gate Phase 13 called for, and ran a policy-level teacher-forcing dose-response study. Six of seven planned tasks completed — Task 6 (Precursor Genome check) was cancelled before any data was downloaded, noted as future work only. **No further Phase 15 experiments are planned.** Full result-by-result breakdown, every result tagged [C]/[E] with n/effect size/CI and a "does not support" line: `docs/phases/PHASE15_SUMMARY.md`.
...
**STATUS: COMPLETE — settled AMPLIFIED at checkpoint 300, see finding 22.**
This section is kept as the original planning record (the run described
below already happened); do not treat its forward-looking language as an
open task.
